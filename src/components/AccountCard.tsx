    ...
    >
      <div className="card-header">
        ...
        <div className="card-info">
          ...
          <div className="card-name">Trae Account</div>
        </div>

        <div className={`card-status ${tokenStatus === "expired" ? "expired" : tokenStatus === "expiring" ? "expiring" : "normal"}`}>
          <span className="status-indicator"></span>
          {tokenStatus === "expired" ? "Expired" : tokenStatus === "expiring" ? "Expiring" : "Normal"}
        </div>
      </div>
      ...
      <div className="card-meta">
        <span className="meta-item">
          ...
          Added {formatCreatedDate(account.created_at)}
        </span>
        <span className="meta-item">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/>
          </svg>
          Reset {usage ? formatDate(usage.reset_time) : "-"}
        </span>
        {usage && usage.extra_expire_time > 0 && (
          <span className="meta-item warning">
            ...
            Gift expires {formatDate(usage.extra_expire_time)}
          </span>
        )}
      </div>
